...
// Read mesh vertices.
void
pylith::meshio::MeshIOPETSc::_readVertices(spatialdata::utils::LineParser& parser,
                                           scalar_array* coordinates,
                                           int* numVertices,
...
// Write mesh vertices.
void
pylith::meshio::MeshIOPETSc::_writeVertices(std::ostream& fileout) const { // _writeVertices
    PYLITH_METHOD_BEGIN;
...
// Read mesh cells.
void
pylith::meshio::MeshIOPETSc::_readCells(spatialdata::utils::LineParser& parser,
                                        int_array* cells,
                                        int_array* materialIds,
                                        int* numCells,
...
// Write mesh cells.
void
pylith::meshio::MeshIOPETSc::_writeCells(std::ostream& fileout) const { // _writeCells
    PYLITH_METHOD_BEGIN;
...
// Read mesh group.
void
pylith::meshio::MeshIOPETSc::_readGroup(spatialdata::utils::LineParser& parser,
                                        int_array* points,
                                        GroupPtType* type,
    ...
    while (buffer.good() && token != "}") {
        if (0 == strcasecmp(token.c_str(), "name")) {
            ...
            *name = cbuffer;
            ...
            buffer.ignore(maxIgnore, '=');
            buffer >> typeName;
            if (typeName == _MeshIOPETSc::groupTypeNames[VERTEX]) {
                *type = VERTEX;
            } else if (typeName == _MeshIOPETSc::groupTypeNames[CELL]) {
                *type = CELL;
            } else {
                std::ostringstream msg;
...
// Write mesh group.
void
pylith::meshio::MeshIOPETSc::_writeGroup(std::ostream& fileout,
    ...
    fileout
        << "  group = {\n"
        << "    name = " << name << "\n"
        << "    type = " << _MeshIOPETSc::groupTypeNames[type] << "\n"
        << "    count = " << numPoints << "\n"
        << "    indices = {\n";
